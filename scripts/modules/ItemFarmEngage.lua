...
-- Parse "cmd1;cmd2;3w" into individual send() calls
...
function M.start(job, merged_resources, on_done, on_failed)
    ...
    ---------------------------------------------------------------------------
    -- Phase: verify_mob (skip for summon mode)
    ---------------------------------------------------------------------------
    if mode ~= "summon" then
    ...
    -- All non-terminal phases --"fail"--> "failed"
    ...
    -- waiting_kill: killed -> kill_confirmed
...
function M.setup_helpers(job, merged_resources)
    ...
    e.evaluate_status = function()
        ...
        if hp_ok and mp_ok then
            ...
            e.echo(string.format("HP/MP insufficient (HP %.0f%% MP %.0f%%), failing", hp_pct, mp_pct))
            mud.sm_transition("itemfarm_engage", "fail")
        end
    end

    -- apply_next_buff: find first missing buff and apply it
    e.apply_next_buff = function()
        ...
        for _, b in ipairs(buffs) do
            ...
            if not hours or hours == 0 then
                ...
                mud.send(b.cmd)
                -- Wait for spell to take effect, then re-check
                mud.timer(2.0, [=[
...
function M.register_event_handlers(job)
    ...
    -- ifarm:mob_fled -> fail (during fighting)
    ...
    -- ifarm:mob_dropped -> killed (charm mode waiting_kill)
