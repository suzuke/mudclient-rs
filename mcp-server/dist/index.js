...
// --- Tool: get_status ---
server.tool("get_status", "取得 MUD client 的連線狀態和 Session 資訊", {}, async () => {
    try {
...
server.tool("read_messages", "讀取 MUD 伺服器最近的訊息。用於觀察遊戲狀態、NPC 對話、戰鬥結果等", {
    ...
    try {
        const data = await apiGet(`/api/messages?count=${count}`);
...
// --- Tool: get_room_info ---
server.tool("get_room_info", "取得當前房間的名稱、出口、Room ID 和描述", {}, async () => {
    try {
        const data = await apiGet("/api/room");
        return {
            content: [
                {
                    type: "text",
...
server.tool("send_command", "發送指令到 MUD 伺服器。可以是遊戲指令（如 look, north, kill mob）或客戶端指令（如 #alias, /lua）", {
    ...
    try {
        const data = await apiPost("/api/send", { command });
        return {
            content: [
                {
                    type: "text",
...
// --- Tool: execute_lua ---
server.tool("execute_lua", "在 MUD client 的 Lua 環境中執行程式碼。可以使用 mud.send()、mud.echo() 等 API", {
    code: z.string().describe("Lua 程式碼，例如 mud.send('look') 或 mud.echo('hello')"),
}, async ({ code }) => {
    try {
        const data = await apiPost("/api/lua", { code });
        return {
            content: [
                {
                    type: "text",
